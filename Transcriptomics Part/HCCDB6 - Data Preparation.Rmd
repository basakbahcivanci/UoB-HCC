...
sample_info = read.csv('HCCDB6.sample.txt',sep = '\t', header = TRUE, row.names = 1)
...
```
```{r}
data = read.csv('GSE14520-GPL3921.gene.txt',sep = '\t',header = TRUE)
data[1:5,1:5]
```
...
### By visual inspection of graph, it can be seen that one Adjacent sample sits far away from each of the clusters. It has been removed with the following codes, since it is a significant outlier.

```{r}
outlier = which(r[,1]< -200 & r[,2]< -200 )
...
pdf(file = "HCCDB6 - PCA- Without Significant outlier.pdf")
fviz_pca_ind(res.pca,
             geom.ind = "point", # show points only (nbut not "text")
             col.ind = groups[-outlier], # color by groups
             addEllipses = TRUE, # Concentration ellipses
...
```{r}
write.csv(data[-outlier,], 'prepared_data6.csv',quote = F,row.names = T)
write.csv(sample_info[-outlier,], 'prepared_sample_info6.csv',quote = F,row.names = F)
```
